...
def every_possible_pairing_of_students(array)
  pairs = []
  for i in 0..2
    pairs << [ array[i] , array[(i + 1) % 3 ] ].sort
    pairs << [ array[i] , array[(i + 2) % 3 ] ].sort
  end
  pairs.sort.uniq
end
...
def get_first_half_of_string(string)
  limit = (string.length.to_f / 2).ceil
...
def titleize_a_string(string)
  string.split.each_with_index do |word, index|
    word.capitalize! unless word.match(/and|the/)
...
def word_count_a_file(file_path)
  File.readlines(file_path).inject(0) do |word_count, line|
    words = line.split
...
def your_birthday_is_on_a_friday_in_the_year(birthday)
  date = DateTime.parse(birthday.to_s)
  date >>= 12 until date.friday?
